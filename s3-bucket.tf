 ...
 policy = <<EOF
{
  ...
  "Statement": [
    ...
    { "Effect": "Allow",
      ...
      "Resource": ["arn:aws:s3:::push-button-demo-bucket"]
},
    { "Effect": "Allow",
      "Action": [
        "s3:PutObject",
...
resource "aws_s3_bucket" "push-button-demo-bucket" {
  bucket = "push-button-demo-bucket"
...
resource "aws_s3_bucket_acl" "demo_bucket_acl" {
  bucket = aws_s3_bucket.push-button-demo-bucket.id
  acl    = "private"
}
